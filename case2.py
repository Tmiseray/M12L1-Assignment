...
1. Outer Loop (for i in range(n)):
    ...
    - This loop primarily serves to control the number of passes over the array. After each pass, the largest unsorted element bubbles up to its correct position, so each pass reduces the size of the unsorted portion of the array by one.
...
3. Comparison and Swap (if arr[j] > arr[j+1] and arr[j], arr[j+1] = arr[j+1], arr[j]):
    ...
    - This is the actual "work" done in the algorithm, as it determines whether elements are out of order and swaps them accordingly.
...
Best case: 
    - In the best case, the input array is already sorted. In this case, Bubble Sort performs n comparisons but no swaps, so the complexity is still O(n²) in the worst case, but in practice, this can be optimized to O(n) by adding a flag to check if any swaps were made in the inner loop (discussed in Task 3).
Worst case: 
    - The worst case occurs when the array is sorted in reverse order. In this case, the algorithm performs the maximum number of comparisons and swaps. Therefore, the complexity is O(n²).

Space Complexity:
-----------------
Bubble Sort is an in-place sorting algorithm, meaning it doesn't require extra space proportional to the size of the input array (other than the space needed for the input array itself).
...
Time Efficiency:
----------------
    - Bubble Sort is inefficient for large datasets because of its O(n²) time complexity. As the input size grows, the number of comparisons and swaps grows quadratically, which makes the algorithm unsuitable for large datasets.
...
1. Optimized Bubble Sort:
    - Adding a flag to check if any swaps occurred during an inner loop pass can improve the best case to O(n) (early termination). This is useful if the input array is partially sorted or nearly sorted.
"""
...
1. Merge Sort (O(n log n)): 
    - Merge Sort is a divide-and-conquer algorithm that has a better time complexity of O(n log n), which is significantly faster than Bubble Sort for large datasets. It works by recursively splitting the array into halves and then merging them back in sorted order.
2. Quick Sort (O(n log n)): 
    - Quick Sort is another divide-and-conquer algorithm that, on average, performs very well with a time complexity of O(n log n). However, it has a worst-case complexity of O(n²), though this can be mitigated with random pivot selection or other optimizations.
3. Insertion Sort (O(n²)): 
    - Insertion Sort also has O(n²) time complexity in the worst case, but it performs well on small or nearly sorted datasets. It may be a better option for smaller arrays compared to Bubble Sort.
...
2. Time Complexity:
    ...
    - Worst Case: O(n²) (for reverse-ordered input).
    - Overall: O(n²) due to the nested loops.
3. Space Complexity:
    - O(1) (in-place sorting with no extra space needed).
4. Efficiency Analysis:
    - Bubble Sort is inefficient for large datasets due to its O(n²) time complexity.
5. Improvements: 
    - Early termination (optimized Bubble Sort) can reduce the best-case time complexity to O(n).
6. Alternatives: 
